...
<div class="d-flex justify-content-end mb-3">
    ...
    <div class="p-2 bd-highlight">C.H. Aprovada: {{$limTF10}}</div>
    <span class="border-right"></span>
    <div class="p-2 bd-highlight">C.H. Restante: {{$chMaxF10-$limTF10}}</div>
</div>
        ...
        @forelse ($dadosForm10 as $dado)
            @if ($dado->usuario_id == $authorized && ($dado->status == "Indeferido" || $dado->status == "Em análise"))
                <tr>
                    ...
                    <td>
                        @can('administrador')
                            <button type="button" class="btn btn-primary"><a class="button-delete-custom" title="Editar" href="{{url('atividades/form10/editar/'.$dado->id.'/'.$dado->usuario_id)}}">
                                Avaliar</a>
                            </button>
                        @endcan
                        @can('normal')
                            ...
                            <div class="modal fade" id="ExemploModalCentralizado10{{$dado->id}}" tabindex="0" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <button type="button" class="btn btn-primary"><a class="button-delete-custom" title="Deletar" href="{{url('atividades/form10/excluir/'.$dado->id)}}">Sim</a></button>
                                    </div>
                                </div>
                                </div>
                            </div>
