<br><br><h4 class="submit_title">Visita Técnica</h4>
...
<div class="table-responsive">
    ...
    <table class="table table table-striped table-bordered" style="width:100%">
        ...
        @forelse ($dadosForm11 as $dado)
            @if ($dado->usuario_id == $authorized)
                <tr>
                    <td>{{$dado->local}}</td>
                    <td>
                        <?php echo date('d/m/Y', strtotime($dado->dt_local)); ?>
                    </td>
                    <td>{{$dado->status}}</td>
                    <td>
                    ...
                    <td>
                        ...
                        @can('normal')
                            ...
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ExemploModalCentralizado11{{$dado->id}}">
                                Excluir
                            </button>
                            <!-- Modal -->
                            <div class="modal fade" id="ExemploModalCentralizado11{{$dado->id}}" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    <h5 class="modal-title" id="TituloModalCentralizado">Visita Técnica: {{$dado->local}}</h5>
                                    ...
                                    <button type="button" class="btn btn-primary"><a class="button-delete-custom" title="Deletar" href="{{url('atividades/form11/excluir/'.$dado->id)}}">Sim</a></button>
                                    </div>
                                </div>
                                </div>
                            </div>
