<br><br><h4 class="submit_title">Estágio Extracurricular: Voluntário no IFNMG</h4>

<div class="d-flex justify-content-end mb-3">
    <div class="p-2 bd-highlight">C.H. Máxima: {{$chMaxF23}}</div>
    <span class="border-right"></span>
    <div class="p-2 bd-highlight">C.H. Aprovada: {{$limTF23}}</div>
    <span class="border-right"></span>
    <div class="p-2 bd-highlight">C.H. Restante: {{$chMaxF23-$limTF23}}</div>
</div>
        ...
        @forelse ($dadosForm23 as $dado)
            @if ($dado->usuario_id == $authorized && ($dado->status == "Indeferido" || $dado->status == "Em análise"))
                <tr>
                    ...
                    <td>
                        @can('administrador')
                            <button type="button" class="btn btn-primary"><a class="button-delete-custom" title="Editar" href="{{url('atividades/form23/editar/'.$dado->id.'/'.$dado->usuario_id)}}">
                                Avaliar</a>
                            </button>
                        @endcan
                        @can('normal')
                            ...
                            <div class="modal fade" id="ExemploModalCentralizado7{{$dado->id}}" tabindex="-1" role="dialog" aria-labelledby="TituloModalCentralizado" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <button type="button" class="btn btn-primary"><a class="button-delete-custom" title="Deletar" href="{{url('atividades/form23/excluir/'.$dado->id)}}">Sim</a></button>
                                    </div>
                                </div>
                                </div>
                            </div>
